...
(TUN) on/off · **x** share config (QR) · **p** proxy help · **q** quit (works even mid-connect).
Config persists in `~/.collateral-config.json`. Mouse tracking hard-locks the frame against
scrolling; hold Option to select text.

### Onboard someone (`x`)

**x** shows a scannable **QR code** of your endpoint plus the `vless://` link. A new user can
scan it with any VLESS client (**v2rayNG · Streisand · Shadowrocket · sing-box**) or paste the
link — no SSH, no first-time setup, they're just pointed at your server. The QR is generated by
a pure, zero-dependency encoder (`common/qr.js`, verified against a reference decoder). Anyone
with the link can use your server, so only share it with people you want on it.
