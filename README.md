...
(TUN) on/off · **x** share config (QR) · **i** import (scan QR) · **p** proxy help · **q** quit
...
- On the machine that's already set up, press **x** — it shows a scannable **QR** of the
  endpoint + key (a `vless://` config, from a pure zero-dependency encoder in `common/qr.js`,
  verified against a reference decoder + OpenCV).
- On the new machine, press **i** — the TUI opens the **Mac camera** (via a tiny Swift helper,
  `mac/scan-qr.swift`, using AVFoundation's built-in QR detection — Node can't touch the
  camera). Point it at the first screen; it decodes, imports the worker URL + key, and you
  press **c** to connect. First use asks for camera permission for your terminal app.

The same QR also imports into any mobile VLESS client (v2rayNG / sing-box / Shadowrocket) if you
ever want that. Anyone with the config can use your server, so only share it with people you
want on it.
